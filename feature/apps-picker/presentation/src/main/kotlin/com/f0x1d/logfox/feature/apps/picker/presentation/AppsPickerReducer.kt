...
internal class AppsPickerReducer @Inject constructor() :
    ...
    override fun reduce(
        ...
        is AppsPickerCommand.BackPressed -> if (state.searchActive) {
            state.copy(searchActive = false).noSideEffects()
        } else {
            state.withSideEffects(AppsPickerSideEffect.PopBackStack)
        }

        is AppsPickerCommand.SearchActiveChanged -> state.copy(searchActive = command.active).noSideEffects()

        is AppsPickerCommand.QueryChanged -> state.copy(query = command.query)
            .withSideEffects(
        ...
        is AppsPickerCommand.AppsLoaded -> state.copy(
            apps = command.apps,
            searchedApps = command.apps,
            isLoading = false,
        ).noSideEffects()

        is AppsPickerCommand.SearchedAppsUpdated -> state.copy(searchedApps = command.apps).noSideEffects()

        is AppsPickerCommand.AppClicked -> state.withSideEffects(
            AppsPickerSideEffect.HandleAppSelection(command.app),
        )
    }
}
