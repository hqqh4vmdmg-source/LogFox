...
internal class PreferencesUIReducer @Inject constructor() :
    ...
    override fun reduce(
        ...
        is PreferencesUICommand.DateFormatChanged -> {
            val format = command.format?.trim() ?: DateTimeSettingsRepository.DATE_FORMAT_DEFAULT
        ...
        is PreferencesUICommand.TimeFormatChanged -> {
            val format = command.format?.trim() ?: DateTimeSettingsRepository.TIME_FORMAT_DEFAULT
        ...
        is PreferencesUICommand.LogsUpdateIntervalChanged -> {
            val interval = command.interval ?: LogsSettingsRepository.LOGS_UPDATE_INTERVAL_DEFAULT
        ...
        is PreferencesUICommand.LogsDisplayLimitChanged -> {
            val limit = command.limit?.coerceAtLeast(0)
