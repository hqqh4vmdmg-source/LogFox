...
internal interface AppCrashDao {
    ...
    fun getAllAsFlow(deleted: Boolean = false): Flow<List<AppCrashRoomEntity>>

    @Query("SELECT * FROM AppCrash WHERE is_deleted = :deleted ORDER BY date_and_time DESC")
    suspend fun getAll(deleted: Boolean = false): List<AppCrashRoomEntity>

    @Query("SELECT * FROM AppCrash WHERE id = :id")
    fun getByIdAsFlow(id: Long): Flow<AppCrashRoomEntity?>

    @Query("SELECT * FROM AppCrash WHERE id = :id")
    suspend fun getById(id: Long): AppCrashRoomEntity?

    @Query("SELECT * FROM AppCrash WHERE package_name = :packageName AND is_deleted = 0")
    suspend fun getAllByPackageName(packageName: String): List<AppCrashRoomEntity>
    ...
    suspend fun getAllByDateAndTime(dateAndTime: Long, packageName: String): List<AppCrashRoomEntity>

    @Insert
    suspend fun insert(appCrash: AppCrashRoomEntity): Long

    @Update
    suspend fun update(appCrash: AppCrashRoomEntity)
    ...
    @Delete
    suspend fun _delete(crashes: List<AppCrashRoomEntity>)
}
